...
class DigActionServer : public rclcpp::Node
{
   ...
   private:
    ...
    rclcpp::Subscription<std_msgs::msg::Float32>::SharedPtr hstp_sub_ =
        this->create_subscription<std_msgs::msg::Float32>(
            "/dig/hstp", 2,
            std::bind(&DigActionServer::dig_hstp_cb, this,
                      std::placeholders::_1));
                  ...
                  double start_time)
    {
        ...
        {
            RCLCPP_ERROR(this->get_logger(),
                         "goto_pos: Goal was out of bounds");
            return;
        }
    ...
    void execute_pos(const std::shared_ptr<GoalHandleDig> goal_handle)
    {
        ...
        { // keep sending the request because CTRE's watchdog
            ...
            linkPercentDone = (abs(lnkage_goal) - abs(current_link_pos_)) /
                              abs(lnkage_goal) * 100;
            bcktPercentDone = (abs(bucket_goal) - abs(current_bckt_pos_)) /
                              abs(bucket_goal) * 100;
            hstpPercentDone = (abs(hrdstp_goal) - abs(current_hstp_pos_)) /
                              abs(hrdstp_goal) * 100;
            vibrPercentDone = 100;
            goal_handle->publish_feedback(feedback);
