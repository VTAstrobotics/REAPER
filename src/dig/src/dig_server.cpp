...
class DigActionServer : public rclcpp::Node
{
   ...
   private:
    ...
    float starting_link_pos_{
        -987654}; // if this is negative 987654 then it means that we have not
    ...
    void execute_pwr(const std::shared_ptr<GoalHandleDig> goal_handle)
    {
        ...
        if (linkage_goal < -1 || linkage_goal > 1 || bucket_goal < -1 ||
            bucket_goal > 1)
        {
            RCLCPP_ERROR(this->get_logger(),
                         "execute_pwr: Linkage and/or Bucket goal was out of "
                         "bounds. Power goals should always be in [-1, 1]");
        }
    ...
    void execute_pos(const std::shared_ptr<GoalHandleDig> goal_handle)
    {
        ...
        { // keep sending the request because CTRE's watchdog
            // TODO: need to factor in the absolute encoders (and any other
            // sensor data) from the callback above
            current_link_pos_ = (double)l_link_mtr_.GetPosition().GetValue();
            current_bckt_pos_ = (double)l_bckt_mtr_.GetPosition().GetValue();
            ...
            RCLCPP_DEBUG(
                this->get_logger(), "Running for %f ms",
                1000 * (1.0 /
                        (double)(LOOP_RATE_HZ_))); // this is the correct math
                                                   // with correct units :)
            ctre::phoenix::unmanaged::FeedEnable(
                1000 * (1.0 / (double)(LOOP_RATE_HZ_)));
            ...
            l_link_pos_duty_cycle_.Velocity =
                linkage_speed;                              // rotations per sec
            ...
            linkPercentDone = (abs(linkage_goal) - abs(current_link_pos_)) /
                              abs(linkage_goal) * 100;
            bcktPercentDone = (abs(bucket_goal) - abs(current_bckt_pos_)) /
                              abs(bucket_goal) * 100;
            goal_handle->publish_feedback(feedback);
