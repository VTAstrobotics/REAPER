...
class DigActionServer : public rclcpp::Node
{
 ...
 private:
  ...
  bool positions_in_bounds( double lnkage_pos, double bucket_pos,
                            double hrdstp_pos )
  {
    return ( linkage_in_bounds( lnkage_pos ) &&
             bucket_in_bounds( bucket_pos ) && hrdstp_in_bounds( hrdstp_pos ) );
  }
  ...
  void execute_pos( const std::shared_ptr<GoalHandleDig> &GOAL_HANDLE )
  {
    ...
    while (
      !APPROX( current_link_pos_, lnkage_goal ) ||
      ...
      link_percent_done = ( abs( lnkage_goal ) - abs( current_link_pos_ ) ) /
                          abs( lnkage_goal ) * 100;
      bckt_percent_done = ( abs( bucket_goal ) - abs( current_bckt_pos_ ) ) /
                          abs( bucket_goal ) * 100;
      hstp_percent_done = ( abs( hrdstp_goal ) - abs( current_hstp_pos_ ) ) /
                          abs( hrdstp_goal ) * 100;
      vibr_percent_done = 100;
      GOAL_HANDLE->publish_feedback( feedback );
