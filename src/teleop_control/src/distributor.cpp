...
class Distributor : public rclcpp::Node
{
    ...
    */
     ...
     * @param buttons is an array of button indices, corresponding to the joy
     * msg
    ...
    void joy1_cb(const sensor_msgs::msg::Joy& raw)
    {
        ...
        auto send_dig_goal_options =
            rclcpp_action::Client<Dig>::SendGoalOptions();
        ...
        if (valid_press(BUTTON_Y, raw))
        {
            ...
            {
                RCLCPP_INFO(this->get_logger(),
                            "Y: Decreasing the max turning rate");
            }
            else
            {
                RCLCPP_INFO(this->get_logger(),
                            "Y: Turning back to full speed");
            }
        }
    ...
    void drive_response_cb(const DriveGoalHandle::SharedPtr& goal_handle)
    {
        ...
        else
        {
            RCLCPP_ERROR(this->get_logger(),
                         "Drive goal was rejected by server");
        }
    }
                     ...
                     const std::shared_ptr<const Drive::Feedback> feedback)
    {
        RCLCPP_INFO(
            ...
            feedback->inst_velocity.angular.y,
            feedback->inst_velocity.angular.z);
    }
    ...
    void dig_result_cb(const DigGoalHandle::WrappedResult& result)
    {
        ...
        RCLCPP_INFO(this->get_logger(),
                    ...
                    result.result->est_dig_link_goal,
                    result.result->est_dig_bckt_goal);
    }
    ...
    void dump_response_cb(const DumpGoalHandle::SharedPtr& goal_handle)
    {
        ...
        {
            RCLCPP_ERROR(this->get_logger(),
                         "Dump goal was rejected by server");
        }
    }
